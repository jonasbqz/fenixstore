...
export default function AdminAccountList({
  ...
  return (
    <div className="space-y-4">
      {/* SECCIÓN Y MODAL DE CONFIGURACIÓN RÁPIDA DE WHATSAPP (CLIENT COMPONENT) */}
      <div className="rounded-3xl border border-[#f5b942]/40 bg-[#0d0f17] p-4 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-[#f5b942]/10 border border-[#f5b942]/40 flex items-center justify-center text-lg shrink-0">
            ⚙️
          </div>
          <div>
            <h3 className="text-xs font-black text-[#f5b942] uppercase tracking-wider">
              Configuración de Grupo de WhatsApp y Teléfono
            </h3>
            <p className="text-[11px] text-zinc-400">
              Modificá directamente el enlace oficial y número de atención desde esta barra.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="h-10 px-4 rounded-xl bg-[#f5b942] hover:bg-[#e0a430] text-xs font-black text-black uppercase tracking-wider transition shadow-gold-glow cursor-pointer w-full sm:w-auto shrink-0"
        >
          ⚙️ Editar WhatsApp
        </button>
      </div>

      {/* MODAL DE EDICIÓN DE CONFIGURACIÓN DE WHATSAPP */}
      {isSettingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl border border-[#f5b942]/40 bg-[#0d0f17] p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[#1f2430] pb-3">
              <h2 className="text-sm font-black text-[#f5b942] uppercase tracking-wider">
                ⚙️ Configurar WhatsApp Oficial
              </h2>
              <button
                onClick={() => setIsSettingsOpen(false)}
                className="rounded-xl border border-[#1f2430] p-1 text-zinc-400 hover:text-white"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveSettings} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-[#1f2430] bg-[#000000] px-3 text-xs font-mono text-white outline-none focus:border-[#f5b942]"
              ...
              <div className="space-y-1">
                <label className="text-xs font-black text-white block">
                  Teléfono de WhatsApp Oficial (con código de país)
                </label>
                <input
                  type="text"
                  required
              ...
              {settingsSavedMsg && (
                <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/40 p-3 text-center text-xs font-black text-emerald-400">
                  ✓ ¡Enlace guardado exitosamente en toda la tienda!
                </div>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setIsSettingsOpen(false)}
                  className="h-11 flex-1 rounded-xl border border-[#1f2430] bg-[#131622] text-xs font-bold text-zinc-400 hover:text-white"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="h-11 flex-1 rounded-xl bg-[#f5b942] hover:bg-[#e0a430] text-xs font-black text-black uppercase tracking-wider shadow-gold-glow disabled:opacity-50"
                >
